...
public class ScanImagesAsyncTask extends AsyncTask<String, ImageData, ArrayList<ImageData>> {
    ...
    public ScanImagesAsyncTask(Context context, Handler handler, ScannerCount scannerCount) {
            ...
            for (int i = 0; i < fileArr.length; i++) {
                ...
                if (fileArr2[i].isDirectory()) {
                    ...
                    if (scanType.equalsIgnoreCase(Constant.PHOTO_SCAN)) {
                        ...
                        if (fileArr2[i].getPath().endsWith(".3gp") || fileArr2[i].getPath().endsWith(".mp4") || fileArr2[i].getPath().endsWith(".mkv") || fileArr2[i].getPath().endsWith(".flv")) {
                            try {
                            File file = new File(fileArr2[i].getPath());
                                int parseInt = 0;
                            try{

                                parseInt = Integer.parseInt(file.length() +"");
                            ...
                            int i2 = this.c++ + 1;
                            video_numArr[0] = i2;
                             scannerCount.scancount(video_numArr);
                             publishProgress(new ImageData(fileArr[i].getPath(), false, file.lastModified(), (long) parseInt));
                    ...
                    } else if (scanType.equalsIgnoreCase(Constant.AUDIO_SCAN)) {

                        if (fileArr2[i].getPath().endsWith(".mp3")) {
